...
int main(int argc, char *argv[]) {
    ...
    // close 后再次调用 close、read、write
    sleep(1);
    printf("close connect fd again, fd=%d\n", connect_fd);
    int result = close(connect_fd);
    printf("result=%d, errno=%d, errmsg=%s\n", result, errno, strerror(errno));

    len = read(connect_fd, snd_buf, 1024);
    printf("len = %d, errno=%d, errmsg=%s\n", len, errno, strerror(errno));

    len = write(connect_fd, "cat", 3);
    printf("len = %d, errno=%d, errmsg=%s\n", len, errno, strerror(errno));
